...
# Stops when it gets a pawn from the other player, but not when it's an empty
# spot
            ...
            if player > 0:
                for (deltaRow, deltaCol) in directions:
                    ...
                    for i in range(4):
                        curRow += deltaRow
                        curCol += deltaCol
                        if getPawn(curRow, curCol) == (3 - player): # XXX wrong here
                            # We got a pawn from the other player
                            break
                    else:
